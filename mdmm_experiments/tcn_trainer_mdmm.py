...
from gnn_tracking.metrics.losses import PotentialLoss, GraphConstructionHingeEmbeddingLoss
...
class TCNTrainer:
    def __init__(
        ...
        main_loss_functions: dict[str, tuple[LossFctType, loss_weight_type]],
        constraint_loss_functions: dict[str, tuple[LossFctType, loss_constraint_type]],
        *,
        device=None,
        lr: Any = 5e-4,
        ...
        for lf in main_loss_functions.values():
            lf[0].to(self.device)
        for lf in constraint_loss_functions.values():
            lf[0].to(self.device)
        ...
        self.main_loss_functions = main_loss_functions
        self.constraint_loss_functions = constraint_loss_functions

        self.l_multipliers_mapper = {}
        i = 0
        for key, (_, these_constraints) in self.constraint_loss_functions.items():
            constraint_loss_dict = unpack_loss_returns(key, these_constraints)
            # self.l_multipliers_mapper.update(unpack_loss_returns(key, these_constraints))
            for k in constraint_loss_dict.keys():
                self.l_multipliers_mapper[k] = int(i)
                i += 1
        
        self.l_multipliers = [nn.Parameter((torch.tensor(0, dtype=torch.float32, device=self.device))) for _ in range(i)]
    ...
    def get_batch_losses(
        ...
        for key, (loss_func, these_weights) in self.main_loss_functions.items():
            ...
            elif isinstance(loss_func, GraphConstructionHingeEmbeddingLoss):
                if embedding_loss is None:
                    embedding_loss = loss_func(**model_output)
                
                for k in these_weights:
                    main_losses.update(unpack_loss_returns(key, {k:embedding_loss[k]}))
        ...
        for key, (loss_func, these_constraints) in self.constraint_loss_functions.items():
            ...
            if isinstance(loss_func, PotentialLoss):
                ...
                for k in these_constraints:
                    constraint_losses.update(unpack_loss_returns(key, {k:embedding_loss[k]}))
        ...
        if torch.isnan(total):
            raise RuntimeError(
                f"NaN loss encountered in test step. {main_losses=}, {main_losses_weights=}, {constraint_losses=}, {constraints=}."
            )
        return total, main_losses, constraint_losses, main_losses_weights, constraints
    ...
    def train_step(self, *, max_batches: int | None = None) -> dict[str, float]:
        ...
        for batch_idx, data in enumerate(self.train_loader):
            ...
            try:
                ...
                batch_loss, main_losses, constraint_losses, main_losses_weights, constraints= self.get_batch_losses(
                    model_output
                )
    ...
    def test_step(self, val=True, max_batches: int | None = None) -> dict[str, float]:
        ...
        for _batch_idx, data in enumerate(loader):
            ...
            for key, value in main_losses.items():
                ...
                batch_metrics[f"{key}_weighted"].append(
                    value.item() * main_losses_weights[key]
                )
        ...
        metrics: dict[str, float] = (
            ...
            | {f"lw_{key}": f[1] for key, f in self.main_loss_functions.items()}
            | {f"lw_{key}": f[1] for key, f in self.constraint_loss_functions.items()}
        )
    ...
    def train(self, epochs=1000, max_batches: int | None = None):
        ...
        for _ in range(1, epochs + 1):
            try:
                result = self.step(max_batches=max_batches)
