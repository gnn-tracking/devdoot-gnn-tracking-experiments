...
class TCNTrainer:
    def __init__(
        ...
        constraint_loss_functions: dict[str, tuple[LossFctType, loss_weight_type]],
        *,
        device=None,
        lr: Any = 5e-4,
        ...
        self.l_multipliers = [torch.zeros((1,), requires_grad=True, device="cuda") for _ in range(i)]
    ...
    def get_batch_losses(
        ...
        for k in constraint_losses:
            infeasibility = constraint_losses[k] - constraints[k]
            damp = 10.0 * (infeasibility**2 / 2) # damping=10
            total += ((self.l_multipliers[self.l_multipliers_mapper[k]][0] * infeasibility) + damp)
